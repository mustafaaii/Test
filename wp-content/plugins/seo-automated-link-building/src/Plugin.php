...
class Plugin
{
    ...
    protected function getStatisticData()
    {
        ...
        foreach($bestResults as $index => $result) {
            $best[] = ['x' => $index, 'y' => $result['cnt'], 'label' => "{$result['title']} (#{$result['link_id']})"];
        }

        $countsIndex = [];
        foreach($data as $d) {
            $countsIndex[$d['y'] . '-' . str_pad($d['m'], 2, '0', STR_PAD_LEFT) . '-' . str_pad($d['d'], 2, '0', STR_PAD_LEFT)] = $d['cnt'];
        }
    ...
    protected function enqueueUserback()
    {
        wp_enqueue_script( 'seo-automated-link-building-userback', plugins_url( '/js/userback.js', $this->name ), array() );
        wp_localize_script( 'seo-automated-link-building-userback', 'seoAutomatedLinkBuildingUserback', array(
            'language' => __('en', 'seo-automated-link-building'),
            'mainButton' => __('Plugin feedback', 'seo-automated-link-building'),
            'ratingHelp' => __('How would you like to rate the plugin?', 'seo-automated-link-building'),
        ));
    }
    ...
    public function updateSettings()
    {
        ...
        Settings::save(array(
            'whitelist' => sanitize_textarea_field($data['whitelist']),
            'blacklist' => sanitize_textarea_field($data['blacklist']),
    ...
    public function changeContent($content)
    {
        ...
        if(!empty($settings['whitelist'])) {
            ...
            foreach($settings['whitelist'] as $whitelistUrl) {
                $parsedUrl = parse_url($whitelistUrl);
        ...
        foreach ($settings['blacklist'] as $blacklistUrl) {
            $parsedUrl = parse_url($blacklistUrl);
            if(!$parsedUrl || !array_key_exists('path', $parsedUrl)) {
                continue;
            }
            // do not change content
        ...
        $links = Link::query()->where('active', true)->get();
        global $post;
        $links = array_filter($links, function(Link $link) use($url, $hostname, $post) {
            if($link->page_id) {
                // link uses post id
            ...
            return strtolower($host) !== strtolower($hostname) || !$this->areUrlsIdentical($path, $url);
        });
